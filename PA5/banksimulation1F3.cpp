...
BankSimulation2::BankSimulation2(){
  customerNum = 0;
  currentTime = 0;
  ...
  idleTime[tellerNums] = {0};
  totalIdleTime[tellerNums] = {0};
}
...
int BankSimulation2::idleServiceWindow( Tellers windows[], int a){
 for(int  i =0; i < a; i++){
...
flog<<std::endl<<"-------------3 tellers with 1 line Simulation Begins------------"<<std::endl<<std::endl;
...
void BankSimulation2::processArrival(Event *&arrivalEvent, EventPQueue &anEventPQueue, BankQueue &aBankQueue){
  std::ofstream flog("PA05.log.txt",std::ofstream::out |std::ofstream::app);
...
void BankSimulation2::processDeparture(Event *&departureEvent, EventPQueue &anEventPQueue, BankQueue &aBankQueue){
std::ofstream flog("PA05.log.txt",std::ofstream::out |std::ofstream::app);
...
anEventPQueue.remove();///< Remove the current event.
  ...
  double BankSimulation2::getSimulationCPUTime()const{
      return simulationCPUTime;
      }
  /// Get the total processing time of event.
  int BankSimulation2::getTotalProcessingTime()const{
      return totalProcessingTime;
      }
  /// Get the average wait time of customer.
  float BankSimulation2::getAverageWaitTime()const{
      return (float)totalWaitTime/customerNum;
      }
  /// Get the maximum wait time of customer.
  int BankSimulation2::getMaxWaitTime()const{
      return MaxWaitTime;
      }
  /// Get the average line length of customer.
  float BankSimulation2::getAverageLineLength()const{
      return (float)totalLineLength/customerNum;
      }
  /// Get the maximum line length of customer.
  int BankSimulation2::getMaxLineLength()const{
      return MaxLineLength;
      }
  /// Get the idle time of teller.
  int* BankSimulation2::getIdleTime(){
      return totalIdleTime;
      }


 void BankSimulation2::informationPrint(){
