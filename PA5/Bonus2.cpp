...
BankSimulationB2::BankSimulationB2(){
 std::cout<<"please enter the number of lineS: ";
  ...
  for(int i = 0; i<lineNums;i++){tellerAvailable[i] = true;idleTime[i] =0;totalIdleTime[i]=0; }

  idleTime = new int[lineNums];
  totalIdleTime = new int[lineNums];
}
...
int BankSimulationB2::getShortestLine(BankQueue aBankQueue[], int lines){
  int shorest, j =0;
...
void BankSimulationB2::simulate(){
...
std::cout<<std::endl<<"-------------"<<lineNums<<" teller with "<<lineNums<<"line Simulation Begins------------"<<std::endl<<std::endl;
...
void BankSimulationB2::processArrival(Event *&arrivalEvent, EventPQueue &anEventPQueue, BankQueue aBankQueue[]){
...
void BankSimulationB2::processDeparture(Event *&departureEvent, EventPQueue &anEventPQueue, BankQueue aBankQueue[]){
...
double BankSimulationB2::getSimulationCPUTime(){
    return simulationCPUTime;
    }
/// Get the total processing time of event.
int BankSimulationB2::getTotalProcessingTime(){
    return totalProcessingTime;
    }
/// Get the average wait time of customer.
float BankSimulationB2::getAverageWaitTime(){
    return (float)totalWaitTime/customerNum;
    }
/// Get the maximum wait time of customer.
int BankSimulationB2::getMaxWaitTime(){
    return MaxWaitTime;
    }
/// Get the average line length of customer.
float BankSimulationB2::getAverageLineLength(){
    return (float)totalLineLength/customerNum;
    }
/// Get the maximum line length of customer.
int BankSimulationB2::getMaxLineLength(){
    return MaxLineLength;
    }
int* BankSimulationB2::getIdleTime(){
    return totalIdleTime;
    }

void BankSimulationB2::informationPrint(){
